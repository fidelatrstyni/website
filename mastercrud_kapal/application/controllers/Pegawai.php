...
class Pegawai extends AUTH_Controller {
	...
	public function index() {
		...
		$data['dataCrew'] = $this->M_pegawai->select_all_pegawai();
		$data['dataKapal'] = $this->M_pegawai->select_kapal();
	...
	public function tampil() {
		$data['dataCrew'] = $this->M_pegawai->select_all();
	...
	public function prosesTambah() {
		...
		$cek = $this->M_pegawai->cekEmail($email);
		...
		if ($cek != 1) {
			...
			if($this->form_validation->run() == TRUE){
				...
				$result = $this->M_pegawai->insert_crew($data);

				if ($result > 0) {
					$out['status'] = '';
					$out['msg'] = show_succ_msg('Data Crew Berhasil ditambahkan', '20px');
					//window.location.reload();
					//redirect('http://localhost/mastercrud_kapal/Pegawai', 'refresh');
				} else {
					$out['status'] = '';
					$out['msg'] = show_err_msg('Data Crew Gagal ditambahkan', '20px');
				}
			}else {
				$out['status'] = 'form';
			...
			$out['status'] = 'email';
			$out['msg'] = show_err_msg('Email Sudah Terdaftar', '20px');
	...
	public function export() {
		...
		$sheet->SetCellValue('D1', "ID Kota");
		$sheet->SetCellValue('E1', "ID Kelamin");
		$sheet->SetCellValue('F1', "ID Posisi");
		...
		foreach($data as $value){
				...
				$sheet->SetCellValue('C'.$rowCount, $value->telp );
				$sheet->SetCellValue('D'.$rowCount, $value->id_kota);
				$sheet->SetCellValue('E'.$rowCount, $value->id_kelamin);
				$sheet->SetCellValue('F'.$rowCount, $value->id_posisi);
				$sheet->SetCellValue('G'.$rowCount, $value->status);
				$rowCount++;
		}

		$writer = new Xlsx($spreadsheet); // instantiate Xlsx
		$filename = 'list-of-pegawai'; // set filename for excel file to be exported
