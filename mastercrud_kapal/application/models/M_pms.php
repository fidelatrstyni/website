...
class M_pms extends CI_Model {
	...
	public function join2table() {
		$this->db->select('*');
		$this->db->from('pms');
		$this->db->join('kapal','kapal.id_kapal = pms.kode_kapal');      
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function insert_pms($data) {
		...
		$sql = "INSERT INTO pms (id_pms, kode_kapal, pemeliharaan, tanggal, aktor) 
				VALUES ('{$data['id_pms']}', '{$data['kode_kapal']}', '{$data['pemeliharaan']}', 
				'{$date}', '{$data['aktor']}' )";
	...
	public function select_by_id($id) {
		$sql = "SELECT * FROM pms WHERE id_pms = {$id}";
...
*/
	...
	public function generateKode() {
		$ci = get_instance();
		$query = "SELECT MAX(id_pms) as maxKode FROM pms";
	...
	public function delete($id) {
		$sql = "DELETE FROM pms WHERE id_pms ='" .$id ."'";
