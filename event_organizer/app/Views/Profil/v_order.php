                    ...
                    <div class="card" style="background-color: #333; color:white;border-color:white">
                        <div class="card-header">
                            <ul class="nav nav-tabs card-header-tabs">
                                <li class="nav-item">
                                    <a style="background-color: #333; color:white;"
                                        class="nav-link <?= ($tittle == 'Proses' ? 'active' : '') ?>"
                                        href="<?= base_url('MyOrder') ?>">
                                        <i class="fa-solid fa-truck-clock"></i>Proses</a>
                                </li>
                                <li class="nav-item">
                                    <a style="background-color: #333; color:white;"
                                        class="nav-link <?= ($tittle == 'Selesai' ? 'active' : '') ?>"
                                        href="<?= base_url('MyOrder/selesai') ?>">Selesai</a>
                                </li>
                                <li class="nav-item">
                                    <a style="background-color: #333; color:white;"
                                        class="nav-link <?= ($tittle == 'Dibatalkan' ? 'active' : '') ?>"
                                        href="<?= base_url('MyOrder/dibatalkan') ?>">Dibatalkan</a>
                                </li>
                            </ul>
                        </div>
                        <div class="container">
                                    ...
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <form action="<?= base_url('MyOrder/pelunasan') ?>" method="POST"
                                                    enctype="multipart/form-data">
                                                    <input type="hidden" name="id" value="<?= $value['id'] ?>">
                                                    <input type="file" name="img_pelunasan">
                                                    <br><br>
                                                    <button type="submit" class="btn text-center"
                                                        style="background-color: black; color:white;"
                                                        value="kirim">Kirim Bukti
                                                    </button>
                                                </form>
                                                <?php } else { ?>
                                                <?php } ?>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <p><?= $value['verifikasi'] ?></p>
                                                <label for="">Bukti Pembayaran</label>
                                                <p><a
                                                        href="<?= base_url('upload/pembayaran/' . $value['img_pembayaran']) ?>">
                                                        <img style="width: 300px; height:200px;"
                                                            src="<?= base_url('upload/pembayaran/' . $value['img_pembayaran']) ?>"
                                                            alt="picture">
                                                    </a></p>
                                                <?php if (!empty($value['img_pelunasan'])) { ?>
                                                <label for="">Bukti Pelunasan</label>
                                                <p><a
                                                        href="<?= base_url('upload/pembayaran/' . $value['img_pelunasan']) ?>">
                                                        <img style="width: 300px; height:200px;"
                                                            src="<?= base_url('upload/pembayaran/' . $value['img_pelunasan']) ?>"
                                                            alt="picture">
                                                    </a></p>
                                                <?php } else { ?>
                                                ...
                                                <p><a
                                                        href="<?= base_url('upload/pembayaran/' . $value['img_pengembalian']) ?>">
                                                        <img style="width: 300px; height:200px;"
                                                            src="<?= base_url('upload/pembayaran/' . $value['img_pengembalian']) ?>"
                                                            alt="picture">
                                                    </a></p>
                                                <?php } else { ?>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <label for="" style="font-size: 20px;">
                                                    Rp.<?= format_number($value['lunas']); ?></label>
                                            </div>
                                            <?php if($tittle == 'Proses') {?>
                                            <div class="form-group">
                                                <form action="<?= base_url('MyOrder/batalkan'); ?>" method="post">
                                                    <input type="hidden" name="id" value="<?= $value['id'] ?>">
                                                    <button type="submit" class="btn btn-danger"
                                                        style="float: right;">Batalkan Pemesanan</button>
                                                </form>
